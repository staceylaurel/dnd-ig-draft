...
const all = () => Query('SELECT discussions.*, player.id FROM discussions JOIN player ON id = discussions.userid');
const one = (id: number) => Query('SELECT discussions.*, player.id FROM discussions JOIN player ON id = discussions.userid WHERE discussions.id = ?', [id]);
